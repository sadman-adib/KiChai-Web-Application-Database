...
if(isset($_SESSION['otp_verified']) && $_SESSION['otp_verified'] && isset($_POST['submit'])){
    ...
    // Generate vendor_ID
    $vendorID = rand(77777,99999); // Generate a random 5-digit ID
    ...
    $sql = "INSERT INTO vendor (vendor_ID, company_name, email, phone, city, post_code, pass, logo, emergency, company_reg_certificate, user_description, per_hour_rate) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt = $conn->prepare($sql);
     ...
     // Bind parameters
     $stmt->bind_param("issssisssssd", $vendorID, $cname, $email, $phone, $city, $post_code, $retype_password, $vendorImage, $emergencyService, $nidFrontPic, $about, $rate);

     if ($stmt->execute()) {
      // Insert interests into the services table
      foreach ($interests as $interest) {
          $insertInterestSql = "INSERT INTO services (vendor_id, service_type) VALUES (?, ?)";
          $stmtInterest = $conn->prepare($insertInterestSql);
          $stmtInterest->bind_param("is", $vendorID, $interest);
