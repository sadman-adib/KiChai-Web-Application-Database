...
if(isset($_SESSION['user_ID'])) {
    $userID = $_SESSION['user_ID'];
        ...
        $sql = "SELECT pname, stype, rating, DateTime, des, taka
        FROM (
            SELECT CONCAT(s.first_name, ' ',s.last_name) AS pname,
                   c.service_type AS stype,
                   r.rating AS rating,
                   c.date_time as DateTime,
                   r.review_description as des,
                   t.amount as taka
            FROM call_outs c
            JOIN specialist s ON c.specialist_id = s.specialist_ID
            LEFT JOIN rating r ON c.job_id = r.job_id
            JOIN transaction t ON c.job_id = t.job_id
            WHERE c.user_id = ? and c.accept = 1 and c.complete = 1
            UNION ALL
  ...
  <div class="container">
    ...
    <div class="scrollable-table-container">
        <?php foreach($data as $row): ?>
            ...
            <div class="city" name="city"><?php echo '<h4> Previous Job Done : </h4>'.$row['DateTime']; ?></div>
            <div class="taka" name="taka"><?php echo '<h4> Transaction : </h4>'.$row['taka']; ?> Taka </div>
            <p class="user-description"><?php echo'<h4> My Review : </h4>'. $row['des']; ?></p>
            </div>
