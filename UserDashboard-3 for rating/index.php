...
if(isset($_SESSION['user_ID'])) {
    ...
    $sql = "SELECT pname, pid, jid, stype, DateTime, em
            FROM (
                SELECT CONCAT(s.first_name, ' ',s.last_name) AS pname,
                       c.specialist_id as pid,
                       c.job_id as jid,
                ...
                WHERE c.user_id = ? and c.accept = 1 
                UNION ALL
                ...
                WHERE c.user_id = ? and c.accept = 1 and c.complete IS NULL
            ) AS combined_data;";
...
if(isset($_POST['continue'])) {
    if(isset($_POST['selected_prof_id'], $_POST['selected_job_id'], $_POST['selected_prof_name'], $_POST['selected_service_type'], $_POST['selected_em'] ) ) {
        $selectedpid = $_POST['selected_prof_id'];
        $selectedjid = $_POST['selected_job_id'];
        ...
        $_SESSION['selected_prof_id'] = $selectedpid;
        $_SESSION['selected_job_id'] = $selectedjid;

        header("Location: http://localhost:3000/add%20review/index.php");
        exit();
    } else {
    ...
    <div class="scrollable-table-container">
        <?php foreach($data as $row): ?>
            ...
            <div class="card-header">
                <span class="pname" name="pname"><h2><?php echo '<h4> Professional Name : </h4>'. $row['pname']; echo '-'; echo $row['pid']; ?></h2> </span>
                <span class="service-type" name="service-type"><?php echo '<h4> Service : </h4> '.$row['stype']; ?></span>
                <div class="card-rating">
                </div>
            </div>
            <div class="card-body">
            <div class="date" name="date"><?php echo '<h4> Date & Time : </h4>'.$row['DateTime']; ?></div>
            <div class="jid" name="jid"><?php echo '<h4> JobID : </h4>'.$row['jid']; ?></div>
            <div class="em" name="em"><?php echo '<h4> </h4>'.$row['em']; ?></div>
            </div>
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                ...
                <!-- Continue button -->
                  <!-- Store selected professional data in session variables -->
                  <input type="hidden" name="selected_job_id" value="<?php echo $row['jid']; ?>">
                ...
                <input type="hidden" name="selected_service_type" value="<?php echo $row['stype']; ?>">

                <button class="see-more" type="submit" name="continue">Complete & Rating</button>
                </form>
            </div>
    </div>
            <?php endforeach; ?>
