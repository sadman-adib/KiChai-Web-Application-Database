...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  move_uploaded_file($_FILES["Vendor-image"]["tmp_name"], $targetDirectory . $vendorImage);
  move_uploaded_file($_FILES["NID-front-pic"]["tmp_name"], $targetDirectory . $nidFrontPic);

  // Store file paths in session variables
  $_SESSION['vendor_image'] = $vendorImage;
  $_SESSION['nid_front_pic'] = $nidFrontPic;
  $_SESSION['about'] = $about;
  $_SESSION['rate'] = $rate;

    // Redirect to another page
    header("Location: http://localhost:3000/Vendor-SignUp%20Page-3/index5.php");
    exit();
}
?>
<!DOCTYPE html>
  ...
  <div class="main-container">
    ...
    <div>
      <form id="signupForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
        <div>
          <div>
            <label for="Vendor-image" class="label-1">Upload your Company logo</label>
            <input class="default-stroke" type="file" name="Vendor-image" id="vendor-image" accept="image/*" >
          </div>
            <label for="NID-front-pic" class="label-2">Upload Company’s TRD license Picture</label>
            <input class="default-stroke-2" type="file" name="NID-front-pic" id="NID-front-pic" accept="image/*">
          </div>
          <textarea class="default-stroke-4" name="about" id="about" rows="1" cols="20" placeholder="Tell us something about the company ..."></textarea>
