...
if (isset($_SESSION['vendor_id'])) {
    $pid = $_SESSION['vendor_id'];
    // Prepare the SQL statement using placeholders for user input
    $sql = "SELECT CONCAT(users.first_name, ' ', users.last_name) AS name, call_outs.job_id as job_id, service_type, call_outs.city AS city, call_outs.post_code AS post_code, call_outs.emergency AS em, call_outs.date_time AS dt, call_outs.description AS des, transaction.amount as taka, rating.rating as rating
    FROM call_outs
    JOIN users ON users.user_ID = call_outs.user_id
    JOIN transaction ON transaction.job_id= call_outs.job_id
    join rating on rating.job_id= call_outs.job_id
    WHERE call_outs.vendor_id = ? AND call_outs.accept = 1 AND call_outs.complete = 1
    Order by taka DESC, 
    rating DESC, 
    dt DESC";
                ...
                <?php foreach ($data as $row) : ?>
                    <div class="card">
                        <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                            ...
                            <div class="card-body">
                                <div class="jid"> <?php echo '<h4> Job ID : </h4>' . $row['job_id']; ?> - Complete</div>
                                <div class="date"> <?php echo '<h4> Date & Time : </h4>' . $row['dt']; ?></div>
                                <div class="city"><?php echo '<h4> City : </h4>' . $row['city']; ?></div>
                                <div class="taka"><?php echo '<h4> Transaction : </h4>' . $row['taka']; ?> Taka </div>
